...
_permission_to_touch_wraps=_thread.allocate_lock()
...
def wrap_around_re(aClass, wildcard, advice):
    ...
    for aMember in dir(aClass):
        ...
        if isinstance(realMember, collections.Callable) and aMember[:6]!="__wrap" and \
...
def with_wrap(advice, *methods, **kwargs):
    ...
    for m in methods:
        if getattr(m, 'im_self', None) != None:
            ...
            # instances=[m.im_self].
            try:
                unbound_m = getattr(m.__self__.__class__, m.__func__.__name__)
            except:
                raise AspectsException("Could not find the unbound version of method %s" % (m,))
            retval.append(_with_wrap(advice, unbound_m, instances=[m.__self__]))
        else: 
    ...
    try: 
        ...
        try:
            ...
            if hasattr(method, 'im_class'): # is this a method of a class?
                replace_method = \
                    lambda orig, new: setattr(method.__self__.__class__, orig, new)
            elif hasattr(method, 'func_globals'): # is this a normal function?
                replace_method = \
                    lambda orig, new: method.__globals__.__setitem__(orig, new)
            else:
...
def peel_around(method):
    ...
    try:
        ...
        methods_name = method.__name__
        methods_class = method.__self__.__class__
